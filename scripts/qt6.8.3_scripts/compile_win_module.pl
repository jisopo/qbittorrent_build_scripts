...
printLineToBat ("$install_dir\\bin\\qt-configure-module.bat $module_dir\\$module_name -- -DQT_NO_MSVC_MIN_VERSION_CHECK=ON && cmake --build . --parallel && cmake --install . --config Release");
