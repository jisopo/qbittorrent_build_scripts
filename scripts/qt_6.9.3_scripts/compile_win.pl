...
my $configure_cmd = "..\\configure -prefix $install_dir -opensource -shared -release -confirm-license -ltcg -make tools -nomake examples -no-dbus -no-icu -system-zlib -openssl-linked -no-gtk -no-opengl -no-opengles3 -sql-sqlite -no-sql-odbc";
...
$configure_cmd .= " -c++std c++20 -openssl-linked -- -DQT_NO_MSVC_MIN_VERSION_CHECK=ON -DCMAKE_CXX_FLAGS=\"/wd4305\" -DZLIB_LIBRARY=\"C:/build/zlib-1.3.1/zlib.lib\" -DOPENSSL_ROOT_DIR=\"C:\\build\\openssl-3.5.4\" -DOPENSSL_INCLUDE_DIR=\"C:\\build\\openssl-3.5.4\\include\" -DOPENSSL_USE_STATIC_LIBS=OFF -DQT_BUILD_TESTS=OFF -DQT_BUILD_EXAMPLES=OFF";
